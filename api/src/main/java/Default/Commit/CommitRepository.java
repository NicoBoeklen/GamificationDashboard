...
public interface CommitRepository extends JpaRepository<Commit, Long>{
    ...
    @Query("SELECT new Default.Commit.Stats.CodeGrowth(CAST(DATE_TRUNC('week', c.date) AS LocalDate), (SUM(c.additions) - SUM(c.deletions))) " +
        "FROM Commit c " +
        "WHERE c.isMerge = false AND c.repositoryId = :repoId " +
    ...
    @Query("SELECT COALESCE(SUM(c.additions - c.deletions),0) " +
        "FROM Commit c " +
        "WHERE c.isMerge = false AND c.date<= :date AND c.repositoryId = :repoId ")
    Long getLoCTillDate(@Param("date") LocalDateTime date, @Param("repoId") Long repoId);
    
    @Query("SELECT COALESCE(SUM(c.additions) - SUM(c.deletions),0) FROM Commit c WHERE c.isMerge = false AND c.repositoryId = :repoId")
    Long getTotalLoC(@Param("repoId") Long repoId);

    @Query("SELECT COALESCE(SUM(c.additions), 0) FROM Commit c WHERE c.isMerge = false AND c.repositoryId = :repoId")
    Long getTotalLoCAdded(@Param("repoId") Long repoId);
    
    @Query("SELECT COALESCE(SUM(c.additions), 0) FROM Commit c WHERE c.isMerge = false AND c.repositoryId = :repoId AND CAST(c.date AS DATE) = :day")
    Long getTotalLoCAddedByDay(@Param("repoId") Long repoId, @Param("day") LocalDate day);

    @Query("SELECT COALESCE(SUM(c.deletions), 0) FROM Commit c WHERE c.isMerge = false AND c.repositoryId = :repoId")
    Long getTotalLoCDeleted(@Param("repoId") Long repoId);

    @Query("SELECT count(c) FROM Commit c WHERE c.isMerge = false AND c.repositoryId = :repoId")
    Integer getTotalCommitCount(@Param("repoId") Long repoId);

    @Query("SELECT count(c) FROM Commit c WHERE c.isMerge = false AND c.repositoryId = :repoId AND CAST(c.date AS DATE) = :day")
