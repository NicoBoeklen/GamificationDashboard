...
public interface PullRequestRepository extends JpaRepository<PullRequest, Long> {
    ...
    @Query("SELECT COUNT(p) FROM PullRequest p WHERE p.closedBy.userId = :userId AND p.openedBy.repoId = :repoId AND FUNCTION('DATE', p.dateClosed) = :day")
    ...
    @Query("SELECT COUNT(p) FROM PullRequest p WHERE p.closedBy.repoId = :repoId AND FUNCTION('DATE', p.dateClosed) = :day")
